...
const Toolbar = ({ page, setPage }) => {
  ...
  const handleCategoryPost = async (e) => {
    ...
    if (category === "all") {
      const getContentData = await axios.get(
        `http://localhost:3700/post/list?page=${page}&lat=${searchLat}&lon=${searchLon}`
      );
      ...
      try {
        const getContentData = await axios.get(
          `http://localhost:3700/post/content/list?page=${page}&category=${category}&lat=${searchLat}&lon=${searchLon}`
        );
        const getPosts = getContentData.data.raws;
        dispatch(setPosts(getPosts));
      } catch (err) {
  ...
  const handleChangeKeyword = async (e) => {
    if (isSearch) {
      try {
        const getContentData = await axios.get(
          `http://localhost:3700/post/content/list?page=${page}&keyword=test14&lat=${searchLat}&lon=${searchLon}`
        );
      } catch (err) {
        console.log(err);
      }
  ...
  return (
    <div className="toolBar">
      ...
      >
        <button className="toolbarButton" onClick={onChangeSearch}>
          🔍︎
        </button>
        <button className="toolbarButton" onClick={onChangeCategory}>
          카테고리
        </button>
